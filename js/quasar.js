			...
			spectra.forEach( (el) => {
				...
				d3.dsv(' ', path + file, (d) => {
					...
					return {
						...
						//'dist_scaled': +spectraScale(d.dist_scaled), //delete when redshift values is added to spectra
...
function createGraph(n_skewers) {
	...
	/*d3.select('#n-graphs').on('change', function(a){
		n_skewers = d3.select(this).property('value');
	})*/
	let n = n_skewers;
	var graphs = [];
	for(i=0;i<n;i++){
