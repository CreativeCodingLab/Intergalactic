			...
			spectra.forEach( (el) => {
				...
				d3.dsv(' ', path + file, (dee) => {
					...
					return {
						...
						// 'absorption': parseFloat(dee.absorption),
					}
				}).then( (data) => {
...
function plotGalaxyImage(){
	...
	var galaxyImage = svg.append('div').style('margin-top', '-5rem')
	galaxyImage.append('img')
		.attr('src', 'data/galaxyImages_partial/'+g.NSAID+'_'+x+'_'+y+'_'+z+'.jpg')
		.attr('width', 200)
		.attr('height', 200)
	galaxyImage.selectAll('p')
		.data([NSAID, pos, rs, lssfr])
		.enter()
			.append("p")
...
function displayGui(){
	...
	skewerFolder.add(guiParams, "skewersVisible").name("Toggle Skewer Visibility");
	skewerFolder.add(guiParams, "textVisible").name("Toggle Text Visibility");
